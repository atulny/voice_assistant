...
def record_audio_chunk(audio, stream, temp_file_path , chunk_length=5):
    print("Recording...")
    frames = []
    ...
    #temp_file_path = './temp_files/temp_audio_chunk.wav'
    print(f"Writing...")
...
def load_prompt():
    friend_name = streamlit.session_state["friend_name"]
    friend_description= streamlit.session_state["friend_description"]

    input_prompt = f"""
    You are a helpful and knowledgeable  expert named {friend_name or 'Taylor Swift'} with the following profile:
    {friend_description or "You are a good friend,"}
...
def play_text_to_speech(text, language='en', temp_audio_file="temp_audio.mp3",slow=False):
